...
public class OrderHistoryViewHandler {
    ...
    public void whenOrderCanceled_then_UPDATE_1(@Payload OrderCanceled orderCanceled) {
        try {
            if (!orderCanceled.validate()) return;
                // view 객체 조회
            Optional<OrderHistory> orderHistoryOptional = orderHistoryRepository.findById(orderCanceled.getId());
    ...
    public void whenOrdered_then_UPDATE_2(@Payload Ordered ordered) {
        try {
            if (!ordered.validate()) return;
                // view 객체 조회
            Optional<OrderHistory> orderHistoryOptional = orderHistoryRepository.findById(ordered.getId());
    ...
    public void whenShipped_then_UPDATE_3(@Payload Shipped shipped) {
        try {
            if (!shipped.validate()) return;
                // view 객체 조회
            Optional<OrderHistory> orderHistoryOptional = orderHistoryRepository.findById(shipped.getOrderId());
    ...
    public void whenShippingCancled_then_UPDATE_4(@Payload ShippingCancled shippingCancled) {
        try {
            if (!shippingCancled.validate()) return;
                // view 객체 조회
            Optional<OrderHistory> orderHistoryOptional = orderHistoryRepository.findById(shippingCancled.getOrderId());
    ...
    public void whenPaymentApproved_then_UPDATE_5(@Payload PaymentApproved paymentApproved) {
        try {
            if (!paymentApproved.validate()) return;
                // view 객체 조회
            Optional<OrderHistory> orderHistoryOptional = orderHistoryRepository.findById(paymentApproved.getOrderid());

            if( orderHistoryOptional.isPresent()) {
                 OrderHistory orderHistory = orderHistoryOptional.get();
            // view 객체에 이벤트의 eventDirectValue 를 set 함
                orderHistory.setApproveStatus("PaymentApproved");    
                // view 레파지 토리에 save
                 orderHistoryRepository.save(orderHistory);
                }
    ...
    public void whenPaymentCanceled_then_UPDATE_6(@Payload PaymentCanceled paymentCanceled) {
        try {
            if (!paymentCanceled.validate()) return;
                // view 객체 조회
            Optional<OrderHistory> orderHistoryOptional = orderHistoryRepository.findById(paymentCanceled.getOrderid());
